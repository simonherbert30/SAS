...
%let infocar=GMF-IT-PP-VDB-2021052;  /*change the month and year*/
LIBNAME it "&path\&quarter";
%LET quarter=2021_Q2; /*change quarter of current RV update*/
%let path=\\itrmewpefp001.eu.ds.ov.finance\SHARED\Residual Values;
%LET verf2ml=f2mQ2_21v3;     	/*Free2Move and PCP new, no interpolation, max 60m*/
%LET verdemo=DEMQ2_21v3;      	/*demo. Need special interpolation*/
%LET verf2mlN1=fN1Q2_21v3;
%LET verPCP=PCPQ2_21v3;
...
if modelName in ('CORSA-E' 'VIVARO50KW-E' 'VIVARO75KW-E' 'ZAFIRALIFE55KW-E' 'ZAFIRALIFE75KW-E' 'NEWMOKKA-E') then gear='A';
...
km_min=30001; km=40000; output;
km_min=40001; km=50000; output;
run;
...
/*************************ADJUSTMENTS FOR 4WD AND AUT**************************/

data VehicleDataBase8;
set	VehicleDataBase7;
RV=RV*100;
format RV 6.2;
if gear = 'A' and transmission = '2WD' and model notin ('CORSA-E' 'VIVARO50KW-E' 'VIVARO75KW-E' 'ZAFIRALIFE55KW-E' 'ZAFIRALIFE75KW-E' 'NEWMOKKA_E') then RV=RV+1;
if gear = 'A' and transmission = '4WD' and model in ('CORSA-E' 'VIVARO50KW-E' 'VIVARO75KW-E' 'ZAFIRALIFE55KW-E' 'ZAFIRALIFE75KW-E' 'NEWMOKKA-E') then RV=RV+1;
if gear = 'A' and transmission = '4WD' and model notin ('CORSA-E' 'VIVARO50KW-E' 'VIVARO75KW-E' 'ZAFIRALIFE55KW-E' 'ZAFIRALIFE75KW-E' 'NEWMOKKA-E') then RV=RV+2;
if gear = 'M' and transmission = '4WD' then RV=RV+1;
run;

/*************************USED FILE**************************/

PROC IMPORT OUT= PCPRVS
DATAFILE= "&path\&quarter\PCP_RVS.xlsx" 
DBMS=Excel REPLACE; SHEET= Feuil1; GETNAMES=YES; delimiter=','; MIXED=NO; SCANTEXT=YES; USEDATE=YES; SCANTIME=YES;
RUN;
...
If km=25000 then km_min=20001;
If km=30000 then km_min=25001;
If km=40000 then km=999999;
if km=999999 then km_min=30001;
run;

data PCPRVS2;
retain manufacturer model codice_infocar fuel km_min km term rv;
set	PCPRVS1;
if fuel="Benzina" then fuel="P";
if fuel="Gasolio" then fuel="D";
if fuel="Elettrica" then fuel="E";
manufacturer="*";
model="";
codice_infocar="";
run;
...
PROC EXPORT DATA= VehicleDataBase10 (rename=(terms=term)) OUTFILE= "&path\DSR Outs\&quarter\&verf2ml" DBMS=TAB;
RUN;
...
where terms not in (6,9,60) and km_original ne 50000;
do i = 0 to 6;
...
where km<>25000 and terms not in (6,9) and rv ne.;
fuel='*';
model=null;
drop modelName segment version gear transmission code variant rv term null manufacturer model fuel;
run;
...
/******************************4. EXPORT PCP RVS********************************/
PROC EXPORT DATA= PCPRVS2 OUTFILE= "&path\DSR Outs\&quarter\&verPCP" DBMS=TAB;
RUN;
